...
const builder: GridBuilder = (grid) => {
	...
	const bottomCells = [
		...
		new CellEditable(grid, [20], 'Petite Suite'),
		new CellEditable(grid, [30], 'Grande Suite'),
	...
	const bottomBonus = new DerivedCell(
		grid,
		derived(
			bottomCells.map((i) => i.score),
			(scores) => {
